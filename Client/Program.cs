...
Console.WriteLine("Client1 waiting...");
Task.Delay(4000).Wait();
Console.WriteLine("Client1 starting...");
var tcpClientWrapper = new TcpClientWrapper("127.0.0.1", 13);
tcpClientWrapper.Open();
Console.WriteLine("Client1 Opnened");
...
while (input.Key != ConsoleKey.Q)
{
    if (input.Key == ConsoleKey.S)
    {
        ...
        var message = "Hello from client1";
        var messageBytes = Encoding.UTF8.GetBytes(message);
        tcpClientWrapper.Write(messageBytes);
        Console.WriteLine($"Client1 sent: \"{message}\"");
    }
...
Console.WriteLine("Client1 Done");
Console.ReadLine();
